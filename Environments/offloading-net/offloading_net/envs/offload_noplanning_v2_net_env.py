...
class offload_noplanning_v2_netEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # 后验奖励计算（在获取实际延迟后）
        if self.total_delay_est >= 0:
            if self.total_delay_est <= self.app_max_delay:
                # 成功处理且未超过最大容忍延迟
                reward = -self.total_delay_est
            else:
                # 处理成功但超过了最大容忍延迟
